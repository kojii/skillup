...
どのファイル名を探すかという内容は設定ファイルに記述されています。初期設定ではindex.phpを最優先で探し、その後index.htmlを探す設定になっています。設定ファイルの変更方法はここでは説明しません。

また、ファイル名を付ける際は半角英数字、-(ハイフン)、_(アンダースコア)のみを使用するようにしましょう。
